...
def _write_pairwise_maf(path: Path, contig: str, ref_seq: str, sample: str, sample_seq: str) -> None:
    path.write_text(
        "##maf version=1\n"
        "a score=0\n"
        f"s {contig} 0 {len(ref_seq)} + {len(ref_seq)} {ref_seq}\n"
...
def test_maf_to_sites_emits_expected_records_and_mask(tmp_path: Path):
    ...
    assert [record[1] for record in all_records] == ["1", "2", "4", "5", "6", "7"]
    assert [record[1] for record in variant_records] == ["2", "5"]
    assert bed == [("chr1", 2, 3), ("chr1", 7, 8)]

    summary = summarize_site_and_mask_coverage(all_sites, masked, tmp_path / "ref.fa.fai", chrom_hint="chr1")
    assert summary["total_bp"] == 8


def test_maf_to_sites_masks_multiallelic_sites_by_default(tmp_path: Path):
    ref = tmp_path / "ref.fa"
    ref.write_text(">chr1\nA\n", encoding="utf-8")
    ...
    summary = summarize_site_and_mask_coverage(all_sites, masked, fai, chrom_hint="chr1")
    assert summary["total_bp"] == 1
