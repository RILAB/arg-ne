...
def test_summary_report_html_contains_contig_and_series(tmp_path: Path):
    ...
    masked = tmp_path / "combined.chr1.masked.bed"
    masked.write_text("chr1\t2\t4\nchr1\t5\t10\n", encoding="utf-8")

    summary = tmp_path / "combined.chr1.site_summary.tsv"
    summary.write_text(
        "metric\tvalue\n"
        "contig\tchr1\n"
        "contig_length\t10\n"
